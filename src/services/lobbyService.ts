...
import { ON_LOBBY_MODE_UPDATED, redisClient, RedisOnGameModeUpdatedNotification, RedisRifts, redisUpdatePlayerRiftsData } from "../config/redis";
...
export async function createLobby(accountId: string, lobbyMode: LOBBY_MODES = LOBBY_MODES.ONE_V_ONE): Promise<Lobby> {
  ...
  if(lobbyMode == LOBBY_MODES.RIFTS){
    logger.info(`Creating RIFT lobby for ${accountId} - matchLobbyId:${lobbyId}`);
    await redisUpdatePlayerRiftsData(accountId, {
      riftLeaderID: newLobby.owner,
      riftLobbyId: newLobby.id,
      riftChapter: "",
      riftKey: "",
      riftHostCharacter: "",
      riftHostSkin: "",
