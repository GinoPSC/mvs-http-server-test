...
export async function handleMatches_id(req: Request<{}, {}, {}, {}>, res: Response) {
  ...
  let GPNum = 544;
  if (profile){
    GPNum = profile.gameplay_preferences;
    await redisUpdatePlayerLoadout(account.id, {
      gameplayPreferences: profile.gameplay_preferences
    } as RedisPlayer);
  };
  ...
  res.send({
    ...
    server_data: {
      ...
      LobbyType: 0,
      ReadyPlayers: {},
      PlayerGameplayPreferences: { [account.id]: GPNum },
...
//FFA WORK IN PROGRESS
export async function handleMatches_matchmaking_ffa_retail_request(req: Request<{}, {}, MATCH_MAKING_REQUEST, {}>, res: Response) {
  const account = req.token;
  const newMatchId = ObjectID().toHexString();
  ...
  logger.info(`data.from_match: (${data.from_match})`);
  logger.info(`data.id: (${data.id})`);
  logger.info(`MATCH_TYPES.FREE_FOR_ALL: (${MATCH_TYPES.FREE_FOR_ALL})`);
  await queueMatch(account.id, [account.id], data.from_match, data.id, MATCH_TYPES.FREE_FOR_ALL);
}

export async function handleMatches_matchmaking_casual_retail_request(req: Request<{}, {}, MATCH_MAKING_REQUEST, {}>, res: Response) {
  const account = req.token;
  const newMatchId = ObjectID().toHexString();
  ...
  await queueMatch(account.id, [account.id], data.from_match, data.id, MATCH_TYPES.CASUAL);
}
